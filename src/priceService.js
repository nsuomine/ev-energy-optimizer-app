...
function tiersHaveOverlap(a, b) {
  ...
  const aWeekdays = Array.isArray(a.weekdays) ? a.weekdays : [];
  const bWeekdays = Array.isArray(b.weekdays) ? b.weekdays : [];
  const sharedWeekdays = aWeekdays.filter((day) => bWeekdays.includes(day));

  if (!sharedWeekdays.length) {
    return false;
  }

  const intervalsForWeekday = (tier, weekday) => {
    const tierWeekdays = Array.isArray(tier.weekdays) ? tier.weekdays : [];
    return (tier.intervals || []).filter((interval) => {
      const days = Array.isArray(interval.days) && interval.days.length ? interval.days : tierWeekdays;
      return days.includes(weekday);
    });
  };

  for (const weekday of sharedWeekdays) {
    const aIntervals = intervalsForWeekday(a, weekday);
    const bIntervals = intervalsForWeekday(b, weekday);

    for (const ia of aIntervals) {
      for (const ib of bIntervals) {
        if (ia.endMinutes > ib.startMinutes && ib.endMinutes > ia.startMinutes) {
          return true;
        }
      }
    }
...
function warnOnTierOverlaps(branchName, tiers) {
  for (let i = 0; i < tiers.length; i += 1) {
    for (let j = i + 1; j < tiers.length; j += 1) {
      if (tiersHaveOverlap(tiers[i], tiers[j])) {
        const aLabel = tiers[i].id || `#${i + 1}`;
        const bLabel = tiers[j].id || `#${j + 1}`;
        console.warn(
          `Hinnoittelun "${branchName}" tasot menevät päällekkäin: ${aLabel} ja ${bLabel}.`
        );
      }
    }
  }
...
function parseCostConfiguration(payload) {
  ...
  warnOnTierOverlaps('siirto', result.siirto.tiers);
  warnOnTierOverlaps('teho', result.teho.tiers);
