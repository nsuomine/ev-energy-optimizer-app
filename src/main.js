...
function readPricingIdFromPath(knownIds) {
  if (typeof window === 'undefined' || !window.location || !knownIds) {
    return '';
  }
  ...
  try {
    candidate = decodeURIComponent(stripped);
  } catch (_) {
    return '';
  }

  return knownIds.has(candidate) ? candidate : '';
}

function pushPricingPath(pricingId, knownIds) {
  if (!canUseHistoryRouting() || !pricingId || !knownIds) {
    return;
  }

  const nextHash = '#' + encodeURIComponent(pricingId);
  const nextUrl = window.location.pathname + window.location.search + nextHash;
  const currentUrl = window.location.pathname + window.location.search + window.location.hash;

  if (nextUrl !== currentUrl) {
    try {
      window.history.pushState({ pricingId }, '', nextUrl);
    } catch (error) {
...
function calculateOptimalPlan(
  ...
  const sliceBranchCost = (branch, start, hours, power) => {
    if (!(hours > 1e-6) || !(power > 0) || !(start instanceof Date)) {
      return { cost: 0, energy: 0, weightedRate: 0 };
    }
    ...
    while (remaining > 1e-6) {
      ...
      const rate = Number(getRateForMoment(branch, cursor)) || 0;
      const sliceEnergy = power * sliceHours;

      cost += sliceEnergy * rate;
      energy += sliceEnergy;
    ...
    return {
      cost,
      energy,
      weightedRate: energy > 0 ? cost / energy : 0
    };
  };
  ...
  for (let i = 0; i < normalized.length; i += 1) {
    ...
    while (energyRemaining > 1e-6 && j < normalized.length) {
      ...
      const siirtoSlice = sliceBranchCost(safeConfig.siirto, entry.start, portionHours, chargingPower);
      const siirtoRate = siirtoSlice.weightedRate;
  ...
  if (bestPlan) {
    const siirtoTiers = Array.isArray(safeConfig.siirto?.tiers) ? safeConfig.siirto.tiers : [];

    if (siirtoTiers.length && bestPlan.startTime && bestPlan.endTime) {
      const missing = [];
      const cursor = new Date(
        bestPlan.startTime.getFullYear(),
        bestPlan.startTime.getMonth(),
        bestPlan.startTime.getDate()
      );
      const endCursor = new Date(
        bestPlan.endTime.getFullYear(),
        bestPlan.endTime.getMonth(),
        bestPlan.endTime.getDate()
      );

      while (cursor.getTime() <= endCursor.getTime()) {
        const dateKey = formatDateKey(cursor);
        const covered = siirtoTiers.some(
          (tier) =>
            typeof tier.startDate === 'string' &&
            typeof tier.endDate === 'string' &&
            dateKey >= tier.startDate &&
            dateKey <= tier.endDate
        );
      ...
      bestPlan.tariffCoverageMissing = missing;
    } else {
      bestPlan.tariffCoverageMissing = [];
    }
  }
...
function SummaryPanel({ plan, chargingTimeHours, loading, error, tehoMode, pricingName }) {
  ...
  const coverageMissing = Array.isArray(plan.tariffCoverageMissing)
    ? plan.tariffCoverageMissing
    : [];
  const coverageWarning = coverageMissing.length
    ? `Siirtohinnasto ei kata päivämääriä ${coverageMissing[0]}${
        coverageMissing.length > 1 ? `–${coverageMissing[coverageMissing.length - 1]}` : ''
      }. Siirtomaksu on arvioitu ilman maksua näiltä päiviltä.`
    : '';
  ...
  return h(
    ...
    coverageWarning ? h('p', { className: 'warning' }, coverageWarning) : null,
    h(
      'div',
      { className: 'metrics' },
      h(
...
function App() {
  ...
  useEffect(() => {
    ...
    async function loadManifest() {
      ...
      try {
        ...
        const pathCandidate = readPricingIdFromPath(knownIds);

        const initialId =
          pathCandidate ||
          (savedCandidate && knownIds.has(savedCandidate) && savedCandidate) ||
          fallbackCandidate;

        if (initialId) {
          pushPricingPath(initialId, knownIds);
        }
  ...
  useEffect(() => {
    if (typeof window === 'undefined' || !pricingOptions.length) {
      return undefined;
    }

    const knownIds = new Set(pricingOptions.map((option) => option.id));

    const handlePopState = () => {
      const pathId = readPricingIdFromPath(knownIds);

      if (pathId && pathId !== selectedPricingId) {
        setSelectedPricingId(pathId);
      }
    };
  ...
  const handlePricingChange = (nextValue) => {
    ...
    if (!knownIds.has(nextValue)) {
      return;
    }

    pushPricingPath(nextValue, knownIds);
  ...
  const handleRefresh = () => {
    ...
    setReloadToken((token) => token + 1);
    setActionMessage('Hintatiedot päivitetään.');
  };
          ...
          h(
            ...
            h(
              ...
              h(
                ...
                {
                  type: 'button',
                  className: 'toolbar-button is-secondary refresh-button',
                  onClick: handleRefresh,
                  disabled: !hasPricing || loading
                },
                'Päivitä hinnat'
              ),
              h(
