...
pricing options appear in the UI and which one is the default. The current
shipping configuration looks like this:
...
```json
{
  ...
  "pricings": [
    {
      "id": "helen",
      "name": "Helen pienjännitetehosiirto (spot)",
      "configPath": "helen.json"
    },
    {
      "id": "caruna",
      "name": "Caruna yleissiirto (spot)",
...
- **Pricing selector** – Choose the distribution company. The page URL updates
  accordingly (e.g. `/helen`, `/caruna`), making it easy to share a link to a
  specific configuration. Reloading the page restores the correct pricing based
  on the path. Hosting the app behind a server that falls back to `index.html`
  for unknown paths is required for deep links to work; when opened directly as
  a `file://` URL the routing is skipped and the saved or default pricing is
  used instead.
- **Refresh prices** – Reloads spot prices and the selected pricing details on
  demand. The app also reloads automatically every 20 minutes.
...
- **Sliders** – Adjust the required energy, charging power and energy margin.
  The margin adds a user-defined premium to the spot price (slider range
  0.00–0.56 snt/kWh, i.e. 0–0.0056 €/kWh) and affects all energy-dependent
  calculations.
